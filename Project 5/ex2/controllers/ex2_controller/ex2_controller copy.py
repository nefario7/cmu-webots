...
print(type(customController).__name__)
...
# reference trajectory
np.save("r_hist_ex2" + "_" + str(lossOfThust), referenceHistory)

# COMMENT one of two following lines to correctly save te state data
if type(customController).__name__ == "AdaptiveController":
    np.save("x_ad_hist_ex2" + "_" + str(lossOfThust), stateHistory)  # states using adatpive controller
elif type(customController).__name__ == "LQRController":
    np.save("x_lqr_hist_ex2" + "_" + str(lossOfThust), stateHistory)  # states using lqr controller

# simulation finished, draw plots
showPlots(lossOfThust)
